...
- [x] **MODL-01**: User can train a BIO token classifier (O, B-REF, I-REF) on gbert-large with a linear classification head
- [x] **MODL-02**: User can enable/disable CRF layer via config toggle to enforce valid BIO transitions
- [x] **MODL-03**: Model uses differential learning rates (lower for BERT encoder, higher for classification head)
- [x] **MODL-04**: Training uses mixed precision (fp16 on CUDA, disabled/bf16 on MPS) with automatic device detection
- [x] **MODL-05**: Training uses gradient clipping to prevent exploding gradients
- [x] **MODL-06**: Training uses linear warmup + linear decay learning rate schedule
- [x] **MODL-07**: User can freeze BERT or apply LoRA via config toggle as alternative to full fine-tuning
- [x] **MODL-08**: Model runs on Apple Silicon (MPS), NVIDIA GPU (CUDA), and CPU with automatic detection
...
- [x] **ENSM-01**: User can enable bagging ensemble via config (n_estimators models with bootstrap resampling)
- [x] **ENSM-02**: First model generates and caches training data; subsequent models resample from cache
- [x] **ENSM-03**: Ensemble inference uses majority vote over BIO predictions at token level
- [x] **ENSM-04**: User can optionally enable gradient-boost-cached variant with error-weighted retraining
